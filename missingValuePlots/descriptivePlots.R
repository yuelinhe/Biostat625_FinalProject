...
hisp_df <- data.frame(group = names(table(data_new$HISPALLP_A)), freq =as.numeric(table(data_new$HISPALLP_A) ))

ggplot(hisp_df, aes(group,freq,fill=group))+geom_col()+theme_bw()+theme(
  axis.text.x = element_blank())+xlab("Race Group")+ylab("count")+
  ggtitle("Race Groups with Hispanic Origin")
#size 650*450


#EDUC_A
educ_df <- data.frame(group = names(table(data_new$EDUC_A)), freq = as.numeric(table(data_new$EDUC_A)))
ggplot(educ_df, aes(group,freq,fill=group))+geom_col()+theme_bw()+theme(
  axis.text.x = element_blank())+xlab("Education Level")+ylab("count")+
...
anx_df <- data.frame(group = names(table(data_new$ANXFREQ_A)), freq = as.numeric(table(data_new$ANXFREQ_A)))
ggplot(anx_df, aes(group,freq,fill=group))+geom_col()+theme_bw()+theme(
  axis.text.x = element_blank())+xlab("Frequency")+ylab("count")+
  ggtitle("How Often Feel Worried, Nervous, or Anxious")


# DEPFREQ_A
dep_df <- data.frame(group = names(table(data_new$DEPFREQ_A)), freq = as.numeric(table(data_new$DEPFREQ_A)))
ggplot(dep_df, aes(group,freq,fill=group))+geom_col()+theme_bw()+theme(
  axis.text.x = element_blank())+xlab("Frequency")+ylab("count")+
  ggtitle("How Often Depressed")
